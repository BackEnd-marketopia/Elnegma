...
@section('title', __('message.Edit Discount'))
...
@section('content')
    <div class="space-y-6 animate-fadeInUp">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <!-- Form Content -->
            <form action="{{ route('admin.discounts.update', [$discount->id, $vendorId]) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }} lg:col-span-2">
                                ...
                                <div class="flex justify-center">
                                    <div class="relative group">
                                        ...
                                        <div class="absolute top-2 right-2">
                                            <div class="w-8 h-8 bg-blue-500 text-white rounded-full text-sm flex items-center justify-center">
                                                <i class="fas fa-check"></i>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <p class="text-center text-xs text-gray-500 mt-2">{{ __('message.Current image') }}</p>
                            </div>
                        @endif
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            {{ __('message.Leave empty to keep current image') }} | {{ __('message.Recommended size') }}: 400x300px | {{ __('message.Max size') }}: 2MB
                        </p>
                        @if ($errors->has('image'))
                            <div class="mt-2 flex items-center text-red-600 text-sm">
                                <i class="fas fa-exclamation-circle mr-2"></i>
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="button" 
                        class="btn btn-danger flex-1 sm:flex-none delete-discount-btn"
                        data-discount-id="{{ $discount->id }}"
                        data-discount-title="{{ $discount->title }}">
                        <i class="fas fa-trash mr-2 rtl:ml-2 rtl:mr-0"></i>
                        {{ __('message.Delete Discount') }}
                    </button>
            ...
            imageInput.addEventListener('change', function(e) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        // Dim the current image if it exists
            ...
            if (resetBtn) {
                resetBtn.addEventListener('click', function(e) {
                    ...
                    Swal.fire({
                        ...
                        text: '{{ __("message.All changes will be lost") }}',
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#dc2626',
                        cancelButtonColor: '#6b7280',
