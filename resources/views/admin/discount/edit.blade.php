...
@section('breadcrumb')
    ...
    <span class="mx-2">/</span>
    <span>{{ __('message.Edit Discount') }}</span>
@endsection
...
<div class="space-y-6 animate-fadeInUp">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <div class="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white">
                        {{ __('message.Edit Discount Information') }}
                    </h3>
                    <p class="text-purple-100 text-sm">
                        {{ __('message.Update the discount details below') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <!-- Form Content -->
        <form action="{{ route('admin.discounts.update', [$discount->id, $vendorId]) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                            ...
                            <div class="mb-4" id="currentImageContainer">
                                <div class="relative inline-block">
                                    <img src="{{ asset($discount->image) }}" 
                                         alt="{{ $discount->title }}" 
                                         class="w-32 h-24 object-cover rounded-xl border-2 border-gray-200 dark:border-gray-600 shadow-lg">
                                    <div class="absolute -bottom-2 -right-2 bg-blue-500 text-white rounded-full px-2 py-1 text-xs">
                        ...
                        <div class="mb-4">
                            <div id="imagePreview" class="hidden">
                                <div class="relative inline-block">
                                    <img id="previewImg" src="" alt="Preview" class="w-32 h-24 object-cover rounded-xl border-2 border-green-300 shadow-lg">
                                    ...
                                    <div class="absolute -bottom-2 -right-2 bg-green-500 text-white rounded-full px-2 py-1 text-xs">
                                        {{ __('message.New') }}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="mt-3 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl">
                            <p class="text-sm text-blue-700 dark:text-blue-300 flex items-center">
                                ...
                                <span>{{ __('message.Leave empty to keep current image') }} | {{ __('message.Recommended size') }}: 400x300px | {{ __('message.Max size') }}: 2MB</span>
                            </p>
                        </div>
                        @if ($errors->has('image'))
                            <div class="mt-3 flex items-center text-red-600 text-sm bg-red-50 dark:bg-red-900/20 p-3 rounded-lg">
            ...
            <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                ...
                <div class="relative">
                    <textarea id="description" 
                              ...
                              placeholder="{{ __('message.Enter discount description') }}">{{ old('description', $discount->description) }}</textarea>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (submitBtn) {
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-3"></i>{{ __("message.Updating") }}...';
