...
@section('content')
                ...
                <div class="flex items-center space-x-3 rtl:space-x-reverse">
                    ...
                    <div>
                        ...
                        <p class="text-purple-100 text-sm">
                            {{ __('message.Update vendor details for') }} {{ $user->name }}
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="form-group {{ $errors->has('name_of_brand_ar') ? 'has-error' : '' }}">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('name_of_brand_ar', $user->vendor->name_ar) }}" 
                                   required
                        ...
                        <div class="form-group {{ $errors->has('name_of_brand_en') ? 'has-error' : '' }}">
                            ...
                            <input type="text" id="name_of_brand_en" name="name_of_brand_en"
                                ...
                                value="{{ old('name_of_brand_en', $user->vendor->name_en) }}" required
                                placeholder="{{ __('message.Enter brand name') . ' ' . __('message.English') }}">
                        ...
                        <!-- Category -->
                        <div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }} lg:col-span-2">
                            ...
                            <select id="category_id" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $user->vendor->category_id) == $category->id ? 'selected' : '' }}>
                                        @if(app()->getLocale() == 'ar')
                        ...
                        <!-- Cities Multi-select -->
                        <div class="form-group {{ $errors->has('city_ids') ? 'has-error' : '' }} lg:col-span-2">
                            ...
                            <select id="city_ids" 
                                ...
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}" 
                                        {{ (old('city_ids') && in_array($city->id, old('city_ids'))) ||
        (isset($user->vendor->citys_id) && in_array($city->id, json_decode($user->vendor->citys_id, true) ?? [])) ? 'selected' : '' }}>
                                        @if(app()->getLocale() == 'ar')
                                            {{ $city->name_arabic }}
                                        @else
                                            {{ $city->name_english }}
                            ...
                            <small class="block text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('message.Hold Ctrl to select multiple cities') }}</small>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                            ...
                            <input type="text" 
                                ...
                                value="{{ old('address', $user->vendor->address) }}" 
                                required
                        ...
                        <div class="form-group {{ $errors->has('google_map_link') ? 'has-error' : '' }}">
                            ...
                            <input type="url" 
                                ...
                                value="{{ old('google_map_link', $user->vendor->google_map_link) }}"
                        ...
                        <div class="form-group {{ $errors->has('whatsapp') ? 'has-error' : '' }}">
                            ...
                            <input type="text" 
                                ...
                                value="{{ old('whatsapp', $user->vendor->whatsapp) }}"
                        ...
                        <div class="form-group {{ $errors->has('facebook') ? 'has-error' : '' }}">
                            ...
                            <input type="url" 
                                ...
                                value="{{ old('facebook', $user->vendor->facebook) }}"
                        ...
                        <!-- Instagram -->
                        <div class="form-group {{ $errors->has('instagram') ? 'has-error' : '' }} lg:col-span-2">
                            ...
                            <input type="url" 
                                ...
                                value="{{ old('instagram', $user->vendor->instagram) }}"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select id="status" 
                                    ...
                                    required>
                                <option value="pending" {{ old('status', $user->vendor->status) == 'pending' ? 'selected' : '' }}>{{ __('message.Pending') }}</option>
                                <option value="accepted" {{ old('status', $user->vendor->status) == 'accepted' ? 'selected' : '' }}>{{ __('message.Accepted') }}</option>
                                <option value="rejected" {{ old('status', $user->vendor->status) == 'rejected' ? 'selected' : '' }}>{{ __('message.Rejected') }}</option>
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('status_of_account') ? 'has-error' : '' }}">
                            <label for="status_of_account" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                                <i class="fas fa-user-check mr-2 text-purple-600"></i>
                                {{ __('message.Account Status') }}
                                <span class="text-red-500">*</span>
                            </label>
                            <select id="status_of_account" 
                                    name="status_of_account" 
                                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 {{ $errors->has('status_of_account') ? 'border-red-500 ring-2 ring-red-200' : '' }}" 
                                    required>
                                <option value="active" {{ old('status_of_account', $user->status) == 'active' ? 'selected' : '' }}>{{ __('message.Active') }}</option>
                                <option value="inactive" {{ old('status_of_account', $user->status) == 'inactive' ? 'selected' : '' }}>{{ __('message.Inactive') }}</option>
                            </select>
                            @if ($errors->has('status_of_account'))
                                <div class="mt-2 flex items-center text-red-600 text-sm">
                                    <i class="fas fa-exclamation-circle mr-2"></i>
                                    {{ $errors->first('status_of_account') }}
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="form-group {{ $errors->has('logo') ? 'has-error' : '' }}">
                            ...
                            <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-xl {{ $errors->has('logo') ? 'border-red-500' : '' }}">
                                <div class="space-y-1 text-center">
                                    @if($user->vendor->logo)
                                    <div id="logo-preview" class="mb-3 flex justify-center">
                                        <img class="h-32 w-32 object-cover rounded-lg" src="{{ asset($user->vendor->logo) }}" alt="Logo preview">
                                    </div>
                                    @else
                        ...
                        <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                            ...
                            <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-xl {{ $errors->has('image') ? 'border-red-500' : '' }}">
                                <div class="space-y-1 text-center">
                                    @if($user->image)
                                    <div id="image-preview" class="mb-3 flex justify-center">
                                        <img class="h-32 w-32 object-cover rounded-lg" src="{{ asset($user->image) }}" alt="Profile image preview">
                                    </div>
                                    @else
                                    <div id="image-preview" class="hidden mb-3 flex justify-center">
                                        <img class="h-32 w-32 object-cover rounded-lg" src="" alt="Profile image preview">
                                    </div>
                                    @endif
                                    <div class="flex text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <div class="form-group {{ $errors->has('cover') ? 'has-error' : '' }}">
                            ...
                            <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-xl {{ $errors->has('cover') ? 'border-red-500' : '' }}">
                                <div class="space-y-1 text-center">
                                    ...
                                    <div id="cover-preview" class="mb-3 flex justify-center">
                                        <img class="h-32 w-full object-cover rounded-lg" src="{{ asset($user->vendor->cover) }}" alt="Cover image preview">
                                    </div>
                                    @else
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="button" class="btn btn-danger flex-1 sm:flex-none delete-vendor-btn" data-vendor-id="{{ $user->id }}"
                        data-vendor-name="{{ $user->name }}">
                        <i class="fas fa-trash mr-2 rtl:ml-2 rtl:mr-0"></i>
                        {{ __('message.Delete vendor') }}
                    </button>
                </div>
            </form>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const deleteBtn = document.querySelector('.delete-vendor-btn');
        if (deleteBtn) {
            deleteBtn.addEventListener('click', function() {
                ...
                Swal.fire({
                    ...
                    html: `{{ __("message.You will not be able to revert this") }}!<br><strong class="text-red-600">${vendorName}</strong>`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        form.method = 'POST';
                        form.action = `{{ url('admin/vendors') }}/${vendorId}`;
                        form.innerHTML = `
                            @csrf
                            @method('DELETE')
                        `;
